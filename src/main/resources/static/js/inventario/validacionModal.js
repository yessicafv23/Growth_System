...
formularioTipo.addEventListener("submit", (e) => {
  if (expresionvalidar.test(document.getElementById("nombreTipo").value)) {
  } else {
    e.preventDefault();
    document
...
formularioInventario.addEventListener("submit", (e) => {
  if (
    expresionvalidar.test(document.getElementById("nombreInv").value)
  ) {
  } else {
    e.preventDefault();
    document
      .getElementById(`nombreInv`)
      .classList.add("modal__input-invalidado");
    document
